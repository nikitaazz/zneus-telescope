...
if __name__ == "__main__":
    # Example usage - basic loading
    print("="*80)
    print("EXAMPLE 1: Basic data loading (no outlier removal)")
    print("="*80)
    df = load_data()
    print("\nDataset info:")
    print(df.info())
    ...
    # Example usage - with outlier removal
    print("\n" + "="*80)
    print("EXAMPLE 2: Data loading with outlier removal")
    print("="*80)
    
    # Option 1: Remove outliers from all numeric columns
    df_clean1, stats1 = load_data(remove_outliers=True)
    
    # Option 2: Remove outliers from specific columns only
    print("\n" + "="*80)
    print("EXAMPLE 3: Remove outliers from specific columns only")
    print("="*80)
    outlier_prone_cols = ['fAsym', 'fWidth', 'fM3Long', 'fLength', 
                          'fM3Trans', 'fConc1', 'fDist', 'fSize']
    df_clean2, stats2 = load_data(
        remove_outliers=True,
        outlier_columns=outlier_prone_cols,
        iqr_multiplier=1.5
    )
    
    # Option 3: More conservative outlier removal (3.0 multiplier)
    print("\n" + "="*80)
    print("EXAMPLE 4: Conservative outlier removal (IQR multiplier=3.0)")
    print("="*80)
    df_clean3, stats3 = load_data(
        remove_outliers=True,
        iqr_multiplier=3.0
    )